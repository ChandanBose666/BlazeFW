...
| **Zero-Fetch Sync** | No more `fetch()` — state syncs over a CRDT-powered WebSocket delta stream |
...
- [Pillar 5 — BlazeFW Inspector](#pillar-5--blazefw-inspector-phase-52-complete)
...
│   ├── vite-plugin/   ← Calls the Rust binary at build time, routes .blazefw.tsx files
...
│   └── a11y/          ← Accessibility runtime utilities, test helpers, blazefw-a11y CLI
└── apps/
...
The Rust compiler analyses every `.blazefw.tsx` file at build time using [SWC](https://swc.rs) and automatically produces two bundles — no annotations required.
...
**`.blazefw/UserDashboard.server.js`** — contains `getUser`, strips `UserCard`

**`.blazefw/UserDashboard.client.js`** — contains `UserCard`, replaces `getUser` with a type-safe RPC bridge:
...
`@blazefw/vite-plugin` intercepts every `.blazefw.tsx` / `.blazefw.ts` file and routes it through the Rust binary (with a WASM fallback when Rust isn't installed) via a `stdin → stdout` JSON bridge. Results are cached per file ID and cleared on HMR update.
...
<Action variant="link" href="https://github.com/ChandanBose666/BlazeFW" external>
  Learn more →
</Action>
```
...
On **web** these resolve to CSS custom properties (`var(--blazefw-primary)`), letting you theme the entire UI by setting variables on `:root`. On **native** and **email** they resolve against a built-in `DEFAULT_THEME` palette (hex strings).

---

### The Three Render Targets

The `BlazeRenderer<TNode>` contract enforces that every target implements all four primitives:
...
## Pillar 5 — BlazeFW Inspector *(Phase 5.2, complete)*
...
Mark components in the DOM using the `data-blazefw-kind` attribute (set automatically by `@blazefw/web` or the compiler):

```tsx
// @blazefw/web adds this automatically based on compiler output
<div data-blazefw-kind="server" data-blazefw-name="UserCard">…</div>
<div data-blazefw-kind="client" data-blazefw-name="Button">…</div>
<div data-blazefw-kind="boundary" data-blazefw-name="useUserData">…</div>
```
...
function Editor() {
  const { value, setValue, boundaryProps } = useSnapshot(initialDoc, { maxSnapshots: 10 });
...
### Task 6.4 — Test Utilities + `blazefw-a11y` CLI
...
#### `blazefw-a11y` CLI — WCAG 2.1 AA compliance reporter

```bash
# Audit a built HTML file
npx blazefw-a11y dist/index.html

# JSON output for CI integration
npx blazefw-a11y dist/index.html --output json

# Non-blocking CI mode (always exit 0)
npx blazefw-a11y dist/index.html --exit-zero

# Custom axe tag filter
npx blazefw-a11y dist/index.html --tags wcag2a,wcag2aa
```
...
blazefw-a11y: ✗ 1 violation — 20 WCAG 2.1 AA manual checks required
```
...
│
│  # (sync pillar) Start the sync server in a separate terminal:
│    node my-app/src/sync-server.js
└
```

> `.blazefw.tsx` files are sliced by `@blazefw/compiler` — a WASM build ships with
> the package, so this works with **zero Rust setup**. Installing [Rust](https://rustup.rs)
> just makes the slicer run a bit faster (the native binary is preferred when present).

The generated project includes:

- `vite.config.ts` wired to `@blazefw/vite-plugin`
- `src/App.blazefw.tsx` — a Dashboard component using semantic primitives, showing a live counter and cards for each active pillar
...
├── packages/
│   ├── compiler/              Rust crate — SWC AST analysis + blazefw-compiler CLI binary
...
│   ├── a11y/                  @blazefw/a11y — Accessibility utilities, test helpers, blazefw-a11y CLI
...
```bash
cd packages/compiler  && cargo test   # Rust slicer + a11y scanner  (44 tests)
cd packages/crdt      && cargo test   # CRDT WASM bridge            (12 tests)
cd packages/web       && pnpm test    # Web renderer                (33 tests)
cd packages/email     && pnpm test    # Email renderer              (52 tests)
cd packages/core      && pnpm test    # useSync hook + loader       (23 tests)
cd packages/sync-server && pnpm test  # WebSocket sync server       (23 tests)
cd packages/sidecar   && pnpm test    # Sidecar Worker              (49 tests)
cd packages/inspector && pnpm test    # BlazeFW Inspector           (59 tests)
cd packages/snapshot  && pnpm test    # Snapshot Boundary           (39 tests)
cd packages/a11y      && pnpm test    # Accessibility Layer         (124 tests)
cd packages/create-blazefw && pnpm test  # Scaffolder                (12 tests)
```
...
| 2.4 | `@blazefw/vite-plugin` + `blazefw-compiler` CLI binary |
...
| 3.4 | `@blazefw/email` — Email renderer (HTML strings, MSO-safe, 52 tests) |
...
| 6.4 | ✅ | `@blazefw/a11y/test` axe-core helpers + `blazefw-a11y` compliance reporter CLI with WCAG coverage map (124 tests) |
