/**
 * Integration tests for BlazeSyncServer.
 *
...
import { BlazeSyncServer, REJECTION_FRAME } from "../sync-server.js";
...
describe("BlazeSyncServer", () => {
  let server: BlazeSyncServer;
  let port: number;
  let baseUrl: string;
  ...
  beforeEach(async () => {
    ...
    server = new BlazeSyncServer({ port });
    await server.ready;
  });
